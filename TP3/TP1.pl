...
/* R�ussit si le premier ensemble inclut le second */
inclut([],[]).
inclut([1|L1],[_|L2]):-
	inclut(L1,L2).
inclut([0|L1],[0|L2]):-
...
/* Produit des listes de composants en panne � partir d'�tats */
convertir(L1,L2):-
	composants(Comp),
...
etat_systeme([EtatA1,EtatA2, EtatM1,EtatM2,EtatM3],[A, B, C, D, E, F, G]) :-
	fd_domain([EtatA1, EtatA2, EtatM1, EtatM2, EtatM3], [0,1]),
	(EtatM1#=0)#==>OutM1#=A*C,
	(EtatM2#=0)#==>OutM2#=B*D,
	(EtatM3#=0)#==>OutM3#=C*E,
	(EtatA1#=0)#==>F#=OutM1+OutM2,
	(EtatA2#=0)#==>G#=OutM2+OutM3,
